...
def load_parameters(file_path):
    ...
    return df.set_index('parameters')['value'].to_dict()

def NPP_Ag_Bg_allocation(NPP,fBNPP,fr20,fr20_100):
	BNPP_parameter_dict = load_parameters('BNPP_parameters.csv')
	fBNPP=BNPP_parameter_dict['fBNPP']
	fr20=BNPP_parameter_dict['fr20']
	fr20_100=BNPP_parameter_dict['fr20_100']
	Cab_npp=NPP*(1-fBNPP)
...
def SoilC_Dynamics_4pools_to_optimize(spin_up_year,NPP,fBNPP,fr20,fr20_100,I_Old_top_down, I_Old_sub_down, k_ab_npp, k_Old_top, k_Old_sub, e_ab_npp_Old):
	...
	NonSen_parameter_dict = load_parameters('NonSen_parameters.csv')
	C_Young_top,C_Old_top=0,0
	C_Young_sub,C_Old_sub=0,0
	I_ab_npp_Young=NonSen_parameter_dict['I_ab_npp_Young']
	I_root_npp_Young_top=NonSen_parameter_dict['I_root_npp_Young_top']
	I_root_npp_Young_sub=NonSen_parameter_dict['I_root_npp_Young_sub']
	I_Young_top_down=NonSen_parameter_dict['I_Young_top_down']
	I_Young_sub_down=NonSen_parameter_dict['I_Young_sub_down']
	k_Young_top_down=NonSen_parameter_dict['k_Young_top_down']
	k_Young_sub_down=NonSen_parameter_dict['k_Young_sub_down']
	e_Young_Old_top=NonSen_parameter_dict['e_Young_Old_top']
	e_Young_Old_sub=NonSen_parameter_dict['e_Young_Old_sub']
	pmax=NonSen_parameter_dict['pmax']
	Km=NonSen_parameter_dict['Km']
...
def load_parameter_boundaries(file_path):
    df = pd.read_csv(file_path)
    lb = df['lower_boundary'].tolist()
    ub = df['upper_boundary'].tolist()
    return lb, ub
...
def parameter_optimize_PSO(spin_up_year,NPP, fBNPP, fr20, fr20_100, observed_C_Young_Old_top_all,observed_C_Young_Old_sub_all):
	...
	lb, ub = load_parameter_boundaries('Sen_parameters.csv')
